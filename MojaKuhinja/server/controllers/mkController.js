...
exports.istaknuto = async(req,res)=>{
    try{
       let count = await jelovnik.find().countDocuments()
       let random = Math.floor(Math.random() * count)
       let jelo = await jelovnik.findOne().skip(random).exec()
       let randomJelo = {jelo}
        
       res.render('jelo-random', {title:'Moja Kuhinja - random', jelo, randomJelo})
        // res.render('jelo', {title:'Moja Kuhinja - jelo ', jelo})
    }catch(error){
        res.status(500).send( {message:error.message || "Nastala je greska!"})
    }
